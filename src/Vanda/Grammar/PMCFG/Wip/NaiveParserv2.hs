...
weightedParse (WPMCFG s rs) bw w = map (\ (Passive _ _ (Derivation t) _) -> t) 
                                    ...
                                    $ solve ds
    where
        ds = DeductiveSolver (Map.empty, Map.empty, Map.empty) update (initialPrediction srules : predictionRule w rs : conversionRule : [completionRule]) bw
        ...
        update (p, a, i) item@(Passive nta _ _ inside) = (updateGroup nta item p, a, updateIfGreater nta inside i)
...
predictionRule word rs = DeductiveRule 1 gets app
  where
    ...
    app :: (Group wt, Eq nt, Hashable nt) 
        ...
        -> [Item nt t wt] 
        -> [(Item nt t wt, wt)]
    app (_, _, insides) [Initial r@(Rule ((_, as), f), w) inside' outside'] = [ (Active r as 0 [] inst inside' outside', inside' <> outside')
...
completionRule = DeductiveRule 2 gets app
  where
    gets :: Container nt t wt -> Item nt t wt -> [[Item nt t wt]]
    gets (passives, _, _)  i@(Active rule' (next:_) _ _ _ _ _) =  [ [i, passive]
    ...
    app :: Container nt t wt -> [Item nt t wt] -> [(Item nt t wt, wt)]
    app (_, _, insides) [Active rule@(_, w) (a:as) offset ds fs ainside outside, Passive _ rv d pinside] =  [ (Active rule as (offset+1) (d:ds) fs' inside outside, inside <> outside)
